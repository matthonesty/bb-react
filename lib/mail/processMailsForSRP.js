    ...
    for (const { mailHeader, mailBody, validation } of validatedMails) {
      ...
      try {
        ...
        if (!validation.valid) {
          ...
          await db.query(
            'INSERT INTO processed_mails (mail_id, from_character_id, sender_name, subject, mail_timestamp, mail_body, status, error_message) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) ON CONFLICT (mail_id) DO NOTHING',
            [mailHeader.mail_id, mailHeader.from, senderName, mailHeader.subject, mailHeader.timestamp, mailBody, 'invalid', `Invalid SRP: ${errorMsg}`]
          );
          results.errors.push({
            mail_id: mailHeader.mail_id,
            subject: mailHeader.subject,
        ...
        if (existingCheck.rows.length > 0) {
          ...
          await db.query(
            ...
            [mailHeader.mail_id, mailHeader.from, senderName, mailHeader.subject, mailHeader.timestamp, mailBody, 'duplicate', existingRequest.id, 'Duplicate killmail']
          );
        ...
        await db.query(
          ...
          [mailHeader.mail_id, mailHeader.from, senderName, mailHeader.subject, mailHeader.timestamp, mailBody, 'srp_created', insertResult.rows[0].id]
        );
