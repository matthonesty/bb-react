...
import { getPendingMailsReadyToSend, markMailSent, updateMailRetry } from '../pendingMailQueue.js';
...
async function sendQueuedMails(accessToken) {
  ...
  console.log(
    `[MAIL QUEUE] Loaded ${Object.keys(approvedShips).length} approved ship types for mail templates`
  );
  ...
  const mailBatch = queuedMails.map((queuedMail) => {
    ...
    return {
      ...
      sendFunction: async () => {
        console.log(
          `[MAIL QUEUE] Sending ${queuedMail.mail_type} mail to ${queuedMail.recipient_character_id} (attempt ${queuedMail.attempts + 1})`
        );
...
<b>O'Bomber-care</b> is Bombers Bar's ship replacement program. Questions? Contact leadership on the Bombers Bar Discord <url=https://discord.gg/yqQFDqRXvr>https://discord.gg/yqQFDqRXvr</url>`.trim(),
          ...
          const paidDate = payload.paidDate
            ? new Date(payload.paidDate).toISOString().split('T')[0]
            : 'Unknown';
          const formattedAmount = payload.paidAmount
            ? payload.paidAmount.toLocaleString()
            : 'Unknown';
...
<b>O'Bomber-care</b> is Bombers Bar's ship replacement program. Questions? Contact leadership on the Bombers Bar Discord <url=https://discord.gg/yqQFDqRXvr>https://discord.gg/yqQFDqRXvr</url>`.trim(),
          ...
          const formattedAmount = payload.payoutAmount
            ? payload.payoutAmount.toLocaleString()
            : 'Unknown';
...
<b>O'Bomber-care</b> is Bombers Bar's ship replacement program. Questions? Contact leadership on the Bombers Bar Discord <url=https://discord.gg/yqQFDqRXvr>https://discord.gg/yqQFDqRXvr</url>`.trim(),
  ...
  for (let i = 0; i < results.length; i++) {
    ...
    if (result.status === 'success') {
      ...
      queueResults.sent++;
      console.log(
        `[MAIL QUEUE] Successfully sent queued mail ${mailJob.queueId} (${mailJob.mailType})`
      );
    } else {
      ...
      if (spamError) {
        ...
        console.log(
          `[MAIL QUEUE] Mail ${mailJob.queueId} still rate limited - retry in ${spamError.retryAfterMinutes} minutes`
        );
      } else {
        ...
        queueResults.failed++;
        console.log(
          `[MAIL QUEUE] Mail ${mailJob.queueId} failed - will retry on next run: ${errorMessage}`
        );
      }
    }
  }
