 ...
 *
 * @example Basic usage
 * import { createAdminEndpoint } from '../../lib/api/adminEndpoint';
 * import { Permissions } from '../../lib/auth/permissions';
 *
 * export default createAdminEndpoint({
...
import { internalError } from '../../lib/utils/errorResponse.js';
...
function createAdminEndpoint(options) {
  ...
  return async (req, res) => {
    try {
      ...
      // Note: db is now the pool, available directly as 'pool' or pass it as 'db' for compatibility
      const context = { auth, db: requireDb ? pool : null };
...
function createCrudEndpoint(options) {
  const { readPermission, writePermission, handlers, errorContext } = options;
...
export { createAdminEndpoint, createGetEndpoint, createCrudEndpoint };
