  ...
  try {
    ...
    if (needsRefresh) {
      try {
        ...
        const testCookie = cookie.serialize(
          'auth_data',
        ...
        res.setHeader(
          'Set-Cookie',
          cookie.serialize('auth_data', JSON.stringify(authData), COOKIE_OPTIONS.SESSION)
        );
      } catch (error) {
    ...
    // All authenticated users have at least 'user' role and can access public features
    const roles = await getRoles(authData.character.id);

    // Return auth data with fresh access token
    // Note: Individual endpoints can check for specific roles (admin, FC, etc.)
    return {
      authenticated: true,
