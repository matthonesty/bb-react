...
import { getWalletJournal, getCharacterCorporation } from '../esi.js';
import { getMailerAccessToken } from '../mailerToken';
import { resolveNames } from '../esi.js';
import { MAILER_CHARACTER_ID } from '../esi.js';
...
async function fetchAndSaveJournalEntries(db, accessToken, corporationId, division = 1) {
  try {
    ...
    console.log(
      `[WALLET Division ${division}] Fetching journal entries${lastId ? ` newer than ${lastId}` : ' (initial fetch)'}`
    );
    ...
    while (hasMore) {
      ...
      try {
        ...
        if (error.message && error.message.includes('Requested page does not exist')) {
          console.log(
            `[WALLET Division ${division}] Reached end of journal pagination at page ${page}`
          );
          hasMore = false;
          break;
        }
    ...
    console.log(
      `[WALLET Division ${division}] Fetched ${allEntries.length} new journal entries from ESI`
    );
    ...
    console.log(
      `[WALLET] Batch inserting ${allEntries.length} entries in ${chunks.length} chunk(s)...`
    );
...
async function reconcileSRPPayments(db) {
  try {
    ...
    console.log(
      `[WALLET] Found ${pendingRequests.rows.length} approved SRP requests awaiting payment`
    );
    ...
    for (const srpRequest of pendingRequests.rows) {
      try {
        ...
        if (journalMatch.rows.length > 0) {
          ...
          console.log(
            `[WALLET] ✓ Reconciled SRP ${srpRequest.id} (killmail: ${srpRequest.killmail_id}) with journal ${journal.id} (reason: ${journal.reason}, amount: ${Math.abs(journal.amount)})`
          );
          reconciled++;
        } else {
          console.log(
  ...
  try {
    ...
    for (let division = 1; division <= 7; division++) {
      try {
        ...
        const journalResult = await fetchAndSaveJournalEntries(
          db,
          accessToken,
          corporationId,
          division
    ...
    console.log(
      `[WALLET] All divisions processed: ${totalSaved} total entries saved, ${totalSkipped} skipped`
    );
