...
import { resolveIds } from './universe.js';
...
/**
 * Search for ships by name using exact name matching
 * Uses POST /universe/ids to resolve ship type names to IDs
 *
 * @param {string} query - Exact ship name to search for
 * @returns {Promise<Array>} Array of type IDs that match
 */
async function searchShips(query) {
  ...
  try {
    ...
    // NOTE: This only works with EXACT names, not partial matches
    const result = await resolveIds([query]);

    // Return inventory_type IDs if found
    if (result && result.inventory_types && result.inventory_types.length > 0) {
      return result.inventory_types.map(item => item.id);
    }
